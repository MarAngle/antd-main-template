...
utils.deepCloneData_old = function (origindata, targetdata, option = {}, currentnum = 1, currentprop = '') {
    ...
    if (option.depth === true || currentnum <= option.depth + 1) {
      ...
      for (let i in origindata) {
        ...
        if (!option.limitData.getLimit(nextprop)) {
          targetdata[i] = this.deepCloneData(origindata[i], targetdata[i], option, currentnum, nextprop)
          if (cache) {
            cache.push(i)
          }
  ...
  for (let n = 0; n < propList.length; n++) {
    if (n < propList.length - 1) {
      ...
      if (!vueSet) {
        ...
        Vue.set(res, propList[n], propData)
      }
    }
  }
}
...
// 根据type设置对象属性值
utils.setStrPropByType = function (item, prop, data, type = 'string', vueSet) {
  let targetdata = this.formatDataByType(data, type)
  this.setPropByStr(item, prop, targetdata, vueSet)
}
